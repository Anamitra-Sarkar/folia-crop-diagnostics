...
# Suppress google-api-core Python version FutureWarning (harmless, cosmetic)
...
async def lifespan(app: FastAPI):
    """
    All heavy initialisation lives here so the container is fully ready
    before HuggingFace's health probe hits port 7860.
    """
    ...
    yield
    # Shutdown cleanup (nothing required)
...
# We explicitly list trusted origins instead.
ALLOWED_ORIGINS = [
    "https://foila-ai.vercel.app",
    "https://folia-ai.vercel.app",
    "http://localhost:5173",
    "http://localhost:3000",
]
...
def diagnose_image(
    ...
    should_run_cloud = (
        request.force_cloud
        or (request.metrics.vacuity > 0.6)
        or (request.metrics.conformal_confidence < 0.75)
    )
