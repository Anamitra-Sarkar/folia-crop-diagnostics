...
def upload_files():
    ...
    loss_img = "/home/anamitra/adaptive-edge-cloud-plant-diagnosis/training/edge_loss_curve.png"
    acc_img = "/home/anamitra/adaptive-edge-cloud-plant-diagnosis/training/edge_accuracy_curve.png"
    readme_path = "/home/anamitra/adaptive-edge-cloud-plant-diagnosis/training/EDGE_README.md"
    ...
    if os.path.exists(loss_img):
        api.upload_file(path_or_fileobj=loss_img, path_in_repo="edge_loss_curve.png", repo_id=EDGE_REPO, token=HF_TOKEN)
        print("Uploaded loss curve to Edge repo.")
    if os.path.exists(acc_img):
        api.upload_file(path_or_fileobj=acc_img, path_in_repo="edge_accuracy_curve.png", repo_id=EDGE_REPO, token=HF_TOKEN)
        print("Uploaded accuracy curve to Edge repo.")
    api.upload_file(path_or_fileobj=readme_path, path_in_repo="README.md", repo_id=EDGE_REPO, token=HF_TOKEN)
    print("Uploaded README to Edge repo.")
    
    # Update Cloud Repo PDF to latest version containing Edge metrics
    print("Syncing updated PDF report to Cloud Model repository...")
    if os.path.exists(pdf_path):
        api.upload_file(path_or_fileobj=pdf_path, path_in_repo="model_report.pdf", repo_id=CLOUD_REPO, token=HF_TOKEN)
        print("Updated PDF report in Cloud repo.")
